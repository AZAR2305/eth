...
function makeFromConfig(config) {
  ...
  var chainFetchers = ChainMap.map(config.chainMap, (function (__x) {
          return ChainFetcher.makeFromConfig(__x, config, targetBufferSize);
        }));
  return {
...
async function makeFromDbState(initialState, config) {
  var isInReorgThreshold = initialState.cleanRun ? false : await DbFunctions.EntityHistory.hasRows(Db.sql);
  ...
  var chainFetchersArr = await Promise.all(Belt_Array.map(initialState.chains, (async function (resumedChainState) {
              ...
              return [
                      chain,
                      await ChainFetcher.makeFromDbState(chainConfig, resumedChainState, initialState.reorgCheckpoints, isInReorgThreshold, config, targetBufferSize)
                    ];
            })));
