...
function make(chainConfig, dynamicContracts, startBlock, endBlock, firstEventBlockNumber, progressBlockNumber, config, targetBufferSize, logger, timestampCaughtUpToHeadOrEndblock, numEventsProcessed, numBatchesFetched, isInReorgThreshold, reorgCheckpoints, maxReorgDepth) {
  ...
  if (registrations !== undefined) {
    var onBlockConfigs$1 = registrations.onBlockByChainId[String(chainConfig.id)];
    if (onBlockConfigs$1 !== undefined) {
      Belt_Array.forEach(onBlockConfigs$1, (function (onBlockConfig) {
    ...
    onBlockConfigs = onBlockConfigs$1;
  } else {
    onBlockConfigs = Js_exn.raiseError("Indexer must be initialized with event registration finished.");
  }
  var fetchState = FetchState.make(startBlock, endBlock, eventConfigs, contracts, config.maxAddrInPartition, chainConfig.id, targetBufferSize, progressBlockNumber, onBlockConfigs, Caml.int_max(!Config.shouldRollbackOnReorg(config) || isInReorgThreshold ? 0 : chainConfig.maxReorgDepth, Belt_Option.getWithDefault(Env.indexingBlockLag, 0)));
  ...
  return {
          ...
          reorgDetection: ReorgDetection.make(chainReorgCheckpoints, maxReorgDepth, Config.shouldRollbackOnReorg(config), undefined),
          safeCheckpointTracking: SafeCheckpointTracking.make(maxReorgDepth, Config.shouldRollbackOnReorg(config), chainReorgCheckpoints)
        };
}

function makeFromConfig(chainConfig, config, targetBufferSize) {
  var logger = Logging.createChild({
        chainId: chainConfig.id
      });
  return make(chainConfig, [], chainConfig.startBlock, chainConfig.endBlock, undefined, -1, config, targetBufferSize, logger, undefined, 0, 0, false, [], chainConfig.maxReorgDepth);
}

async function makeFromDbState(chainConfig, resumedChainState, reorgCheckpoints, isInReorgThreshold, config, targetBufferSize) {
...
async function getLastKnownValidBlock(chainFetcher, getBlockHashesOpt) {
  ...
  var scannedBlockNumbers = ReorgDetection.getThresholdBlockNumbers(chainFetcher.reorgDetection, chainFetcher.currentBlockHeight);
  ...
  var fallback = async function () {
    var match = await getBlockHashes$1([getHighestBlockBelowThreshold(chainFetcher)]);
    if (match.length !== 1) {
      return Js_exn.raiseError("Unexpected case. Failed to fetch block data for the last block outside of reorg threshold during reorg rollback");
    } else {
      return match[0];
    }
  };
  ...
  var blockRef;
  var retryCount = 0;
  while(Belt_Option.isNone(blockRef)) {
    var blockNumbersAndHashes = await getBlockHashes$1(scannedBlockNumbers);
    var block = ReorgDetection.getLatestValidScannedBlock(chainFetcher.reorgDetection, blockNumbersAndHashes, chainFetcher.currentBlockHeight, retryCount > 2);
    if (block.TAG === "Ok") {
      blockRef = block._0;
    } else if (block._0 === "NotFound") {
      blockRef = await fallback();
    } else {
      Logging.childTrace(chainFetcher.logger, "Failed to find a valid block to rollback to, since received already reorged hashes from another HyperSync instance. HyperSync has multiple instances and it's possible that they drift independently slightly from the head. Indexing should continue correctly after retrying the query in " + String(100) + "ms.");
