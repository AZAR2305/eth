...
function startServer(getState, config, param) {
  var app = Express();
  ...
  app.get("/console/state", (function (_req, res) {
          res.json(S$RescriptSchema.reverseConvertToJsonOrThrow(getState(), stateSchema));
        }));
  app.post("/console/syncCache", (function (_req, res) {
          Persistence.getInitializedStorageOrThrow(config.persistence).dumpEffectCache().then(function () {
                res.json(true);
              });
        }));
...
async function main() {
  try {
    ...
    var config = RegisterHandlers.registerAllHandlers();
    var gsManagerRef = {
      contents: undefined
    };
    ...
    Prometheus.RollbackEnabled.set(config.historyConfig.rollbackFlag === "RollbackOnReorg");
    startServer(shouldUseTui ? (function () {
    ...
    await Persistence.init(config.persistence, ChainMap.values(config.chainMap), undefined);
    var chainManager = await ChainManager.makeFromDbState(Persistence.getInitializedState(config.persistence), config);
    var globalState = GlobalState.make(config, chainManager, shouldUseTui);
    ...
    var gsManager = GlobalStateManager.make(stateUpdatedHook, globalState);
