// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var Db = require("./db/Db.res.js");
var IO = require("./IO.res.js");
var Env = require("./Env.res.js");
var Types = require("./Types.res.js");
var Utils = require("envio/src/Utils.res.js");
var Config = require("./Config.res.js");
var Hrtime = require("envio/src/bindings/Hrtime.res.js");
var Logging = require("envio/src/Logging.res.js");
var $$Promise = require("envio/src/bindings/Promise.res.js");
var ChainMap = require("envio/src/ChainMap.res.js");
var Benchmark = require("./Benchmark.res.js");
var PgStorage = require("envio/src/PgStorage.res.js");
var Belt_Array = require("rescript/lib/js/belt_Array.js");
var EventUtils = require("envio/src/EventUtils.res.js");
var Prometheus = require("envio/src/Prometheus.res.js");
var Caml_option = require("rescript/lib/js/caml_option.js");
var Persistence = require("envio/src/Persistence.res.js");
var UserContext = require("./UserContext.res.js");
var ChainFetcher = require("./eventFetching/ChainFetcher.res.js");
var ErrorHandling = require("envio/src/ErrorHandling.res.js");
var InMemoryTable = require("./InMemoryTable.res.js");
var Caml_exceptions = require("rescript/lib/js/caml_exceptions.js");
var S$RescriptSchema = require("rescript-schema/src/S.res.js");
var Caml_js_exceptions = require("rescript/lib/js/caml_js_exceptions.js");

function allChainsEventsProcessedToEndblock(chainFetchers) {
  return Belt_Array.every(ChainMap.values(chainFetchers), (function (cf) {
                return ChainFetcher.hasProcessedToEndblock(cf);
              }));
}

function computeChainsState(chainFetchers) {
  var chains = {};
  Belt_Array.forEach(ChainMap.entries(chainFetchers), (function (param) {
          var chainId = String(param[0]);
          var isReady = param[1].timestampCaughtUpToHeadOrEndblock !== undefined;
          chains[chainId] = {
            isReady: isReady
          };
        }));
  return chains;
}

function convertFieldsToJson(fields) {
  if (fields === undefined) {
    return {};
  }
  var keys = Object.keys(fields);
  var $$new = {};
  for(var i = 0 ,i_finish = keys.length; i < i_finish; ++i){
    var key = keys[i];
    var value = fields[key];
    $$new[key] = typeof value === "bigint" ? value.toString() : value;
  }
  return $$new;
}

function addItemToRawEvents(eventItem, inMemoryStore) {
  var $$event = eventItem.event;
  var block = $$event.block;
  var logIndex = $$event.logIndex;
  var blockNumber = eventItem.blockNumber;
  var chain = eventItem.chain;
  var eventConfig = eventItem.eventConfig;
  var eventId = EventUtils.packEventIndex(blockNumber, logIndex);
  var blockFields = convertFieldsToJson(block);
  var transactionFields = convertFieldsToJson($$event.transaction);
  Types.Block.cleanUpRawEventFieldsInPlace(blockFields);
  var params = S$RescriptSchema.reverseConvertOrThrow($$event.params, eventConfig.paramsRawEventSchema);
  var params$1 = params === null ? "null" : params;
  var rawEvent_event_name = eventConfig.name;
  var rawEvent_contract_name = eventConfig.contractName;
  var rawEvent_src_address = $$event.srcAddress;
  var rawEvent_block_hash = block.hash;
  var rawEvent_block_timestamp = eventItem.timestamp;
  var rawEvent = {
    chain_id: chain,
    event_id: eventId,
    event_name: rawEvent_event_name,
    contract_name: rawEvent_contract_name,
    block_number: blockNumber,
    log_index: logIndex,
    src_address: rawEvent_src_address,
    block_hash: rawEvent_block_hash,
    block_timestamp: rawEvent_block_timestamp,
    block_fields: blockFields,
    transaction_fields: transactionFields,
    params: params$1
  };
  var eventIdStr = eventId.toString();
  InMemoryTable.set(inMemoryStore.rawEvents, {
        chainId: chain,
        eventId: eventIdStr
      }, rawEvent);
}

var ProcessingError = /* @__PURE__ */Caml_exceptions.create("EventProcessing.ProcessingError");

async function runEventHandlerOrThrow(item, checkpointId, handler, inMemoryStore, loadManager, persistence, shouldSaveHistory, shouldBenchmark, chains) {
  var timeBeforeHandler = Hrtime.makeTimer();
  try {
    var contextParams = {
      item: item,
      checkpointId: checkpointId,
      inMemoryStore: inMemoryStore,
      loadManager: loadManager,
      persistence: persistence,
      isPreload: false,
      shouldSaveHistory: shouldSaveHistory,
      chains: chains,
      isResolved: false
    };
    await handler({
          event: item.event,
          context: UserContext.getHandlerContext(contextParams)
        });
    contextParams.isResolved = true;
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    throw {
          RE_EXN_ID: ProcessingError,
          message: "Unexpected error in the event handler. Please handle the error to keep the indexer running smoothly.",
          exn: exn,
          item: item,
          Error: new Error()
        };
  }
  if (!shouldBenchmark) {
    return ;
  }
  var timeEnd = Hrtime.floatFromMillis(Hrtime.toMillis(Hrtime.timeSince(timeBeforeHandler)));
  return Benchmark.addSummaryData("Handlers Per Event", item.eventConfig.contractName + " " + item.eventConfig.name + " Handler (ms)", timeEnd, 4);
}

async function runHandlerOrThrow(item, checkpointId, inMemoryStore, loadManager, config, shouldSaveHistory, shouldBenchmark, chains) {
  if (item.kind === 0) {
    var handler = item.eventConfig.handler;
    if (handler !== undefined) {
      await runEventHandlerOrThrow(item, checkpointId, handler, inMemoryStore, loadManager, config.persistence, shouldSaveHistory, shouldBenchmark, chains);
    }
    if (config.enableRawEvents) {
      return addItemToRawEvents(item, inMemoryStore);
    } else {
      return ;
    }
  }
  var match = item.onBlockConfig;
  try {
    var contextParams = {
      item: item,
      checkpointId: checkpointId,
      inMemoryStore: inMemoryStore,
      loadManager: loadManager,
      persistence: config.persistence,
      isPreload: false,
      shouldSaveHistory: shouldSaveHistory,
      chains: chains,
      isResolved: false
    };
    await match.handler({
          block: {
            number: item.blockNumber,
            chainId: match.chainId
          },
          context: UserContext.getHandlerContext(contextParams)
        });
    contextParams.isResolved = true;
    return ;
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    throw {
          RE_EXN_ID: ProcessingError,
          message: "Unexpected error in the block handler. Please handle the error to keep the indexer running smoothly.",
          exn: exn,
          item: item,
          Error: new Error()
        };
  }
}

async function preloadBatchOrThrow(batch, loadManager, persistence, inMemoryStore, chains) {
  var promises = [];
  var itemIdx = 0;
  for(var checkpointIdx = 0 ,checkpointIdx_finish = batch.checkpointIds.length; checkpointIdx < checkpointIdx_finish; ++checkpointIdx){
    var checkpointId = batch.checkpointIds[checkpointIdx];
    var checkpointEventsProcessed = batch.checkpointEventsProcessed[checkpointIdx];
    for(var idx = 0; idx < checkpointEventsProcessed; ++idx){
      var item = batch.items[itemIdx + idx | 0];
      if (item.kind === 0) {
        var handler = item.eventConfig.handler;
        if (handler !== undefined) {
          try {
            promises.push($$Promise.silentCatch(handler({
                          event: item.event,
                          context: UserContext.getHandlerContext({
                                item: item,
                                checkpointId: checkpointId,
                                inMemoryStore: inMemoryStore,
                                loadManager: loadManager,
                                persistence: persistence,
                                isPreload: true,
                                shouldSaveHistory: false,
                                chains: chains,
                                isResolved: false
                              })
                        })));
          }
          catch (exn){
            
          }
        }
        
      } else {
        var match = item.onBlockConfig;
        try {
          promises.push($$Promise.silentCatch(match.handler({
                        block: {
                          number: item.blockNumber,
                          chainId: match.chainId
                        },
                        context: UserContext.getHandlerContext({
                              item: item,
                              checkpointId: checkpointId,
                              inMemoryStore: inMemoryStore,
                              loadManager: loadManager,
                              persistence: persistence,
                              isPreload: true,
                              shouldSaveHistory: false,
                              chains: chains,
                              isResolved: false
                            })
                      })));
        }
        catch (exn$1){
          
        }
      }
    }
    itemIdx = itemIdx + checkpointEventsProcessed | 0;
  }
  await Promise.all(promises);
}

async function runBatchHandlersOrThrow(batch, inMemoryStore, loadManager, config, shouldSaveHistory, shouldBenchmark, chains) {
  var itemIdx = 0;
  for(var checkpointIdx = 0 ,checkpointIdx_finish = batch.checkpointIds.length; checkpointIdx < checkpointIdx_finish; ++checkpointIdx){
    var checkpointId = batch.checkpointIds[checkpointIdx];
    var checkpointEventsProcessed = batch.checkpointEventsProcessed[checkpointIdx];
    for(var idx = 0; idx < checkpointEventsProcessed; ++idx){
      var item = batch.items[itemIdx + idx | 0];
      await runHandlerOrThrow(item, checkpointId, inMemoryStore, loadManager, config, shouldSaveHistory, shouldBenchmark, chains);
    }
    itemIdx = itemIdx + checkpointEventsProcessed | 0;
  }
}

function registerProcessEventBatchMetrics(logger, loadDuration, handlerDuration, dbWriteDuration) {
  Logging.childTrace(logger, {
        msg: "Finished processing batch",
        loader_time_elapsed: loadDuration,
        handlers_time_elapsed: handlerDuration,
        write_time_elapsed: dbWriteDuration
      });
  Prometheus.incrementLoadEntityDurationCounter(loadDuration);
  Prometheus.incrementEventRouterDurationCounter(handlerDuration);
  Prometheus.incrementExecuteBatchDurationCounter(dbWriteDuration);
}

async function processEventBatch(batch, inMemoryStore, isInReorgThreshold, loadManager, config, chainFetchers) {
  var totalBatchSize = batch.totalBatchSize;
  var chains = computeChainsState(chainFetchers);
  var logger = Logging.getLogger();
  Logging.childTrace(logger, {
        msg: "Started processing batch",
        totalBatchSize: totalBatchSize,
        chains: Utils.Dict.mapValues(batch.progressedChainsById, (function (chainAfterBatch) {
                return {
                        batchSize: chainAfterBatch.batchSize,
                        progress: chainAfterBatch.progressBlockNumber
                      };
              }))
      });
  try {
    var timeRef = Hrtime.makeTimer();
    if (Utils.$$Array.notEmpty(batch.items)) {
      await preloadBatchOrThrow(batch, loadManager, config.persistence, inMemoryStore, chains);
    }
    var elapsedTimeAfterLoaders = Hrtime.intFromMillis(Hrtime.toMillis(Hrtime.timeSince(timeRef)));
    if (Utils.$$Array.notEmpty(batch.items)) {
      await runBatchHandlersOrThrow(batch, inMemoryStore, loadManager, config, Config.shouldSaveHistory(config, isInReorgThreshold), Env.Benchmark.shouldSaveData, chains);
    }
    var elapsedTimeAfterProcessing = Hrtime.intFromMillis(Hrtime.toMillis(Hrtime.timeSince(timeRef)));
    var executeBatch = async function (escapeTables) {
      var val;
      try {
        val = await IO.executeBatch(Db.sql, batch, inMemoryStore, isInReorgThreshold, config, escapeTables);
      }
      catch (raw_exn){
        var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
        if (exn.RE_EXN_ID === Persistence.StorageError) {
          return {
                  TAG: "Error",
                  _0: ErrorHandling.make(exn.reason, logger, exn.message)
                };
        }
        if (exn.RE_EXN_ID !== PgStorage.PgEncodingError) {
          return {
                  TAG: "Error",
                  _0: ErrorHandling.make(exn, logger, "Failed writing batch to database")
                };
        }
        var escapeTables$1 = escapeTables !== undefined ? Caml_option.valFromOption(escapeTables) : new Set();
        escapeTables$1.add(exn.table);
        return await executeBatch(Caml_option.some(escapeTables$1));
      }
      var elapsedTimeAfterDbWrite = Hrtime.intFromMillis(Hrtime.toMillis(Hrtime.timeSince(timeRef)));
      var handlerDuration = elapsedTimeAfterProcessing - elapsedTimeAfterLoaders | 0;
      var dbWriteDuration = elapsedTimeAfterDbWrite - elapsedTimeAfterProcessing | 0;
      registerProcessEventBatchMetrics(logger, elapsedTimeAfterLoaders, handlerDuration, dbWriteDuration);
      if (Env.Benchmark.shouldSaveData) {
        Benchmark.addEventProcessing(totalBatchSize, elapsedTimeAfterLoaders, handlerDuration, dbWriteDuration, elapsedTimeAfterDbWrite);
      }
      return {
              TAG: "Ok",
              _0: undefined
            };
    };
    return await executeBatch(undefined);
  }
  catch (raw_exn){
    var exn = Caml_js_exceptions.internalToOCamlException(raw_exn);
    if (exn.RE_EXN_ID === ProcessingError) {
      return {
              TAG: "Error",
              _0: ErrorHandling.make(exn.exn, Logging.getItemLogger(exn.item), exn.message)
            };
    }
    throw exn;
  }
}

exports.allChainsEventsProcessedToEndblock = allChainsEventsProcessedToEndblock;
exports.computeChainsState = computeChainsState;
exports.convertFieldsToJson = convertFieldsToJson;
exports.addItemToRawEvents = addItemToRawEvents;
exports.ProcessingError = ProcessingError;
exports.runEventHandlerOrThrow = runEventHandlerOrThrow;
exports.runHandlerOrThrow = runHandlerOrThrow;
exports.preloadBatchOrThrow = preloadBatchOrThrow;
exports.runBatchHandlersOrThrow = runBatchHandlersOrThrow;
exports.registerProcessEventBatchMetrics = registerProcessEventBatchMetrics;
exports.processEventBatch = processEventBatch;
/* Db Not a pure module */
