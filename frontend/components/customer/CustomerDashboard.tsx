...
export default function CustomerDashboard() {
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <nav className="relative z-10 p-4 md:p-6 flex flex-col md:flex-row justify-between items-center border-b border-white/10 gap-4">
        <div className="flex gap-4 md:gap-6 flex-wrap justify-center">
          <Link href="/" className="text-xl md:text-2xl font-bold text-cyan-300 hover:scale-105 transition">
            🎬 MOVIEX
          </Link>
          <Link href="/customer" className="link-accent transition text-sm md:text-base">📽️ Browse</Link>
          <Link href="/customer/tickets" className="text-cyan-300 hover:text-cyan-200 transition text-sm md:text-base">🎫 My Tickets</Link>
          <Link href="/customer/pyusd-setup" className="text-yellow-400 hover:text-yellow-300 transition text-sm md:text-base">💰 PYUSD Setup</Link>
        </div>
        <WalletConnect />
      </nav>
      ...
      <main className="relative z-10 container mx-auto px-4 md:px-6 py-8 md:py-12">
        {!isConnected && (
          <div className="mb-6 md:mb-8 rounded-xl border border-white/10 bg-white/5 p-4 flex items-center justify-between gap-4">
            <div>
              <p className="font-semibold">Browse showtimes without a wallet</p>
  ...
  <h2 className="text-3xl md:text-4xl font-bold mb-6 md:mb-8 text-cyan-300">🎞️ Available Showtimes</h2>
        
        {loading ? (
          <div className="text-center py-16">
            <p className="text-xl text-gray-400">Loading showtimes from blockchain...</p>
          </div>
        ) : shows.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {shows.filter(s => s && s.id !== undefined && s.movieId !== undefined).map((show) => {
              ...
              <Link 
                ...
                className="block"
              >
                <div className="panel p-6 hover:bg-white/10 transition cursor-pointer">
                  ...
                  <h3 className="text-2xl font-bold mb-2">{show.movie?.title || 'Unnamed Movie'}</h3>
